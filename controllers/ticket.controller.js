...
const buyTicket2 = async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ error: "Invalid or missing price for paid tickets" });
  }
  ...
  try {
    ...
    const event =
      (await Event.findOne({ event_id: eventId })) ||
    ...
    if (event.current_attendee_list.includes(userId)) {
      return res
        .status(401)
        .json({ error: "You already the ticket for this event." });
    }
    ...
    if (ticketType === "RSVP") {
      ...
      await ticket.save();
      const ticketdetails = await Ticket.findById(ticket._id).populate(
        "event_id"
      );
...
const scanTicket = async (req, res) => {
  ...
  try {
    let ticket;
    try {
      ticket = await Ticket.findById(ticketId).populate("event_id");
    } catch (err) {
      return res.status(404).json({ error: "Ticket not found" });
    }
    if (!ticket) {
...
const requestRefund = async (req, res) => {
  ...
  try {
    ...
    if (ticket.payment_status !== "Paid") {
      return res
        .status(400)
        .json({ error: "Refund can only be requested for paid tickets" });
    }
